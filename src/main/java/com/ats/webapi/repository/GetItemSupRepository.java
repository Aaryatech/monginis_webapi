...
public interface GetItemSupRepository extends JpaRepository<GetItemSup, Integer>{
	...
	@Query(value="Select s.id,s.item_id,i.item_name,s.item_hsncd,s.item_uom,s.actual_weight,s.base_weight,s.input_per_qty,s.is_gate_sale,s.is_gate_sale_disc,s.is_allow_bday,s.del_status from m_item_sup s,m_item i where s.item_id=i.id AND s.del_status=:delStatus order by i.item_name Asc",nativeQuery=true)
