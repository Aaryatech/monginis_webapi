...
public interface GetGateEmployeeRepository extends JpaRepository<GetGateSaleEmp, Integer>{
	...
	@Query(value="Select e.emp_id ,e.emp_name,e.emp_type,e.dept_id,d.dept_name,e.is_used,e.del_status,e.emp_mobile,e.emp_dob,e.emp_doj,e.emp_fam_memb,e.disc_id,dis.discount_head,dis.discount_per,e.monthly_limit,e.yearly_limit,coalesce((select SUM(t_gatesale_bill_header.bill_grant_amt) from t_gatesale_bill_header where t_gatesale_bill_header.category=1 AND  t_gatesale_bill_header.category=1 AND t_gatesale_bill_header.bill_date between (select DATE_FORMAT(NOW(), '%y-%m-01')) And :lastDateOfMonth And t_gatesale_bill_header.emp_id=e.emp_id  ),0)as monthly_consumed,coalesce((select SUM(t_gatesale_bill_header.bill_grant_amt) from t_gatesale_bill_header where t_gatesale_bill_header.category=1 AND t_gatesale_bill_header.bill_date between  (select DATE_FORMAT(NOW(),'%Y-01-01')) And (select DATE_FORMAT(NOW(),'%Y-12-31')) And t_gatesale_bill_header.emp_id=e.emp_id  ),0)as yearly_consumed from m_emp e,m_cm_dept d ,m_gatesale_discount dis where  e.dept_id=d.dept_id AND e.disc_id=dis.discount_id AND  e.del_status=0" + 
			"",nativeQuery=true)
