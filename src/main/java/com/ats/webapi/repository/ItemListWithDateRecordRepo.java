...
public interface ItemListWithDateRecordRepo extends JpaRepository<ItemListWithDateRecord, Integer>{
	...
	@Query(value="select distinct t.item_id,i.item_name  from t_order t,m_item i where delivery_date between :fromDate "
			+ "and :toDate and i.id = t.item_id AND t.fr_id IN(:frIds) order by i.item_grp1,i.item_grp2,i.item_name",nativeQuery=true)
	List<ItemListWithDateRecord> getOrderReportByDateAndFr(@Param("fromDate") String fromDate,@Param("toDate") String toDate,@Param("frIds") List<Integer> frIds);
