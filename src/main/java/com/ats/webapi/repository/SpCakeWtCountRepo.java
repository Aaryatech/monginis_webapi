...
public interface SpCakeWtCountRepo extends JpaRepository<SpCakeWtCount, Integer> {
	...
	@Query(value = "SELECT t_sp_cake.sp_selected_weight as sp_selected_wt, COUNT(t_sp_cake.sp_selected_weight) as count FROM t_sp_cake WHERE t_sp_cake.del_status=0  AND t_sp_cake.menu_id IN(:menuIdList) AND t_sp_cake.sp_delivery_date  BETWEEN :fromDate AND :toDate GROUP BY t_sp_cake.sp_selected_weight", nativeQuery = true)
