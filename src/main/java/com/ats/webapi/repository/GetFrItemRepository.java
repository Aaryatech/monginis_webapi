...
public interface GetFrItemRepository extends  CrudRepository<ItemWithSubCat, Long>{
	 ...
	 @Query(value = "SELECT m_item.* ,m_cat_sub.sub_cat_name FROM m_item , m_cat_sub WHERE item_id IN(:items) AND m_cat_sub.sub_cat_id=m_item.item_grp2", nativeQuery = true)
