...
public interface GetFrItemRepository extends  CrudRepository<ItemWithSubCat, Long>{
	 ...
	 @Query(value = " SELECT m_item.* , m_cat_sub.sub_cat_name,0 as qty_limit FROM m_item , m_cat_sub WHERE m_item.id IN(:items) AND m_cat_sub.sub_cat_id = m_item.item_grp2 AND m_item.del_status=0 and item_is_used=1 order by item_sort_id", nativeQuery = true)
