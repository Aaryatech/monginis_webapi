...
public interface StoreStockDetailRepository extends JpaRepository<StoreStockDetail, Integer>{
	...
	@Query(value=" SELECT d.store_stock_detail_id, d.store_stock_id, d.store_stock_date, d.rm_id, d.rm_name,"
			...
			+" t_store_stock_detail t, t_store_stock_header h where d.store_stock_id=h.store_stock_id AND"
			+" t.store_stock_id=h.store_stock_id AND f.store_stock_id=h.store_stock_id AND h.store_stock_date BETWEEN :fromDate"
			+ " AND :toDate AND h.store_stock_status=1 AND f.store_stock_date=:fromDate AND"
			+" t.store_stock_date=:toDate AND t.rm_id=d.rm_id AND f.rm_id=d.rm_id GROUP BY d.rm_id",nativeQuery=true)
	List<StoreStockDetail> getMonthWiseSoreStock(@Param("fromDate")String fromDate, @Param("toDate")String toDate);
