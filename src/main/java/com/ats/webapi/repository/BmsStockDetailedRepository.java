...
public interface BmsStockDetailedRepository extends JpaRepository<BmsStockDetailed,Long>{
	...
	@Query(value=" SELECT b.bms_stock_deatil_id, b.is_del_status, b.rm_type, b.rm_name, b.rm_id, b.rm_uom, b.bms_stock_date, b.bms_stock_id, b.bms_opening_stock, sum(d.store_rec_qty) as store_rec_qty, sum(d.store_rejected_qty) as store_rejected_qty, sum(d.mixing_rec_qty) as mixing_rec_qty,"
			...
			+" t_bms_stock_details b, t_bms_stock_details m WHERE "
				+ "d.bms_stock_date BETWEEN :fromDate AND :toDate AND d.rm_type=:rmType AND b.bms_stock_date =:fromDate "
			+" AND b.rm_type=:rmType AND m.bms_stock_date=:toDate AND m.rm_type=:rmType group by d.rm_id "
				+ "",nativeQuery=true)
