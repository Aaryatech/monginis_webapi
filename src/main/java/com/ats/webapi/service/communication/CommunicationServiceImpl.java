...
public class CommunicationServiceImpl implements CommunicationService {
			...
			if (notificationRes == null) {
				...
				try {
					for (String token : frTokens) {
						if (token != null) {
							Firebase.sendPushNotifForCommunication(token,
									notificationRes.getSubject() + "#" + userRes.getUsername(), jsonStr, "nf");
						}
					}
				} catch (Exception e) {
					e.printStackTrace();
			...
			if (suggestionRes == null) {
				...
				try {
					for (String token : usrTokens) {
						...
						Firebase.sendPushNotifForCommunication(token,
								suggestionRes.getTitle() + "#" + allFrIdName.getFrName(), jsonStr, "s");
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
			...
			if (complaintRes == null) {
				...
				try {
					for (String token : usrTokens) {
						...
						Firebase.sendPushNotifForCommunication(token,
								complaintRes.getTitle() + "#" + allFrIdName.getFrName(), jsonStr, "c");
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
			...
			if (feedbackRes == null) {
				...
				try {
					for (String token : frTokens) {
						...
						Firebase.sendPushNotifForCommunication(token,
								feedbackRes.getTitle() + "#" + userRes.getUsername(), jsonStr, "f");
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
				...
				if (suggestionDetailRes.getIsAdmin() == 0) {
					...
					try {
						for (String token : usrTokens) {
							...
							Firebase.sendPushNotifForCommunication(token, "Suggestion" + "#" + allFrIdName.getFrName(),
									jsonStr, "sd");
						}
					} catch (Exception e) {
						e.printStackTrace();
					...
					String frToken = suggestionDetailRepository
							.findFrTokenBySuggestionId(suggestionDetailRes.getSuggestionId());
					...
					try {
						for (String token : usrTokens) {
							...
							Firebase.sendPushNotifForCommunication(token, "Suggestion" + "#" + userRes.getUsername(),
									jsonStr, "sd");
						}
					} catch (Exception e) {
						e.printStackTrace();
				...
				if (complaintDetailRes.getIsAdmin() == 0) {
					...
					try {
						for (String token : usrTokens) {
							...
							Firebase.sendPushNotifForCommunication(token,
									complaintDetailRes.getMessage() + "#" + allFrIdName.getFrName(), jsonStr, "cd");
						}
					} catch (Exception e) {
						e.printStackTrace();
					}
					...
					String frToken = complaintDetailRepository
							.findFrTokenByComplaintId(complaintDetailRes.getComplaintId());
					...
					try {
						for (String token : usrTokens) {
							...
							Firebase.sendPushNotifForCommunication(token,
									complaintDetailRes.getMessage() + "#" + userRes.getUsername(), jsonStr, "cd");
						}
					} catch (Exception e) {
						e.printStackTrace();
					}
				...
				if (feedbackDetailRes.getIsAdmin() == 0) {
					...
					try {
						for (String token : usrTokens) {
							...
							Firebase.sendPushNotifForCommunication(token,
									feedbackDetailRes.getMessage() + "#" + allFrIdName.getFrName(), jsonStr, "fd");
						}
					} catch (Exception e) {
						e.printStackTrace();
					}
					...
					try {
						for (String token : usrTokens) {
							...
							Firebase.sendPushNotifForCommunication(token,
									feedbackDetailRes.getMessage() + "#" + userRes.getUsername(), jsonStr, "fd");
						}
					} catch (Exception e) {
						e.printStackTrace();
					}
	...
	public List<GetSuggestion> getAllSuggestions() {
		...
		try {
			...
			for (int i = 0; i < allSuggestions.size(); i++) {
				List<GetSuggestionDetail> suggestionDetails = getSuggestionDetailRepository
						.findBySuggestionId(allSuggestions.get(i).getSuggestionId());
	...
	public List<GetComplaint> getAllComplaints() {
		...
		try {
			...
			for (int i = 0; i < allComplaints.size(); i++) {
				List<GetComplaintDetail> complaintDetails = getComplaintDetailRepository
						.findByComplaintId(allComplaints.get(i).getComplaintId());
	...
	public List<GetFeedback> getAllFeedback() {
		...
		try {
			...
			for (int i = 0; i < feedbackList.size(); i++) {
				List<GetFeedbackDetail> feedbackDetailList = getFeedbackDetailRepository
						.getAllDetailsByFeedbackId(feedbackList.get(i).getFeedbackId());
	...
	public List<GetFeedback> getAllFeedbackByFrId(int frId) {
		...
		try {
			...
			for (int i = 0; i < feedbackList.size(); i++) {
				List<GetFeedbackDetail> feedbackDetailList = getFeedbackDetailRepository
						.getAllDetailsByFrIdAndFeedbackId(frId, feedbackList.get(i).getFeedbackId());
				feedbackList.get(i).setGetFeedbackDetails(feedbackDetailList);
			}
		} catch (Exception e) {
			e.printStackTrace();
