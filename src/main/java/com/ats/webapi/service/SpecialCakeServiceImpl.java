...
import com.ats.webapi.repository.GetSpCakeSupCatRepository;
import com.ats.webapi.repository.GetSpCakeSupRepository;
import com.ats.webapi.repository.OrderSpCakeRepository;
import com.ats.webapi.repository.SpCakeSupCatRepository;
...
public class SpecialCakeServiceImpl implements SpecialCakeService {
	...
	@Autowired
	GetSpCakeSupCatRepository getSpCakeSupCatRepository;

	@Autowired
	SpCakeSupCatRepository spCakeSupCatRepository;
	...
	@Override
	public SpCakeSupplementCat saveSpCakeSupCat(SpCakeSupplementCat spCakeSupplementCat) {

		SpCakeSupplementCat spCakeSupplementRes = spCakeSupCatRepository.saveAndFlush(spCakeSupplementCat);
		return spCakeSupplementRes;
	}
	...
	@Override
	public GetSpCkSupplement getSpCakeSuppCat(int id) {

		GetSpCkSupplement spCakeSupplement = getSpCakeSupRepository.findByIdForCat(id);
		return spCakeSupplement;
	}
	...
	public List<GetSpCkSupplementCat> getSpCakeSupCatList() {
		List<GetSpCkSupplementCat> spSuppList;
		try {
			spSuppList = getSpCakeSupCatRepository.findByDelStatus(0);

		} catch (Exception e) {
			spSuppList = new ArrayList<GetSpCkSupplementCat>();
			e.printStackTrace();
		}
		return spSuppList;
	}
